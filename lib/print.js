...
export default async function handleMessage(m, conn = { user: {} }) {
  ...
  if ((opts?.antibot || global.db?.data?.chats?.[m.chat]?.antibot) && m.isGroup && m.sender) {
    const msgId = m.msg?.id || m.key.id;
    if (msgId && ["BAE", "3EB0"].some(p => msgId.startsWith(p)) && m.sender !== conn.user.jid) {
      await m.reply("✦— *Anti bot*\n> Group ini dilengkapi dengan anti bot!\n> Anda melanggar peraturan bot");
      if (conn.logger) conn.logger.info("Bot detected: " + m.sender.split("@")[0]);
